public class DrivingMovement implements Movement {
	...
	public String moveTo(Position from, Position to) {
		...
		return "driving from " + from + " to " + to
			...
			+ ",\nat a speed between " + Utils.roundTo(this.minimumSpeed, 2) + " and " + Utils.roundTo(this.maximumSpeed, 2)
			+ ",\nwhich will take between " + Utils.roundTo(minimumTime, 2) + " and " + Utils.roundTo(maximumTime, 2) + " seconds"
			+ ",\nbut will on average take " + Utils.roundTo(averageTime, 2) + " seconds";
	}
}
